...
{
    [ValidateInput(false)]
    public class UserHIController : BaseController
    {
        ...
        public UserHIController(ICityRepository cityRepository, IUserRepository userRepository, IJobRepository jobRepository, ISkillRepository skillRepository, ICourseRepository courseRepository, IJobRoleRepository jobRoleRepository, IChatRepository chatRepository, IUserJobRepository userJobRepository)
        {
        ...
        public ActionResult ValidateTempPassword(string TemporaryPassword, string MobileNo, string userAgent)
        {
            ...
            return RedirectToAction("ResetPassword", "uhi");
        }
        ...
        public ActionResult SaveResetPassword(UserDTO model)
        {
            try
            {
                ...
                if (msg == true)
                {
                    ...
                    return RedirectToAction("Login", "hi");
                }
            }
            catch (Exception ex)
            {
        ...
        public ActionResult SaveChangePassword(ChangePasswordDTO model)
        {
            ...
            try
            {
                ...
                {
                    return RedirectToAction("ChangePassword", "uhi");
                }
            }
            catch (Exception ex)
            {
        ...
        public ActionResult UserProfile(string Id)
        {
            ...
            if (Session["Id"] != null)
            {
                ...
                ViewBag.Course = _courseRepository.GetAll(x => x.IsDeleted == false).ToList();
                ViewBag.Skill = _skillRepository.GetAll(x => x.IsDeleted == false).ToList();
                ViewBag.City = _cityRepository.GetAll(x => x.IsDeleted == false).ToList();
                ...
                if (model.User != null)
                {
                    ...
                    if (model.UserDetail != null)
                    {
                        ...
                        {
                            model.City = model.UserDetail.City.NameH;
                        }
